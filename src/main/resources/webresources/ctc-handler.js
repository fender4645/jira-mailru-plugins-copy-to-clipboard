AJS
		...
		.ready(
				function() {
					JIRA.bind(JIRA.Events.NEW_CONTENT_ADDED, function(e,
							context, reason) {
						bindCtc();
					});
					bindCtc();
					...
					function initCopyToClipboard() {
						...
						var swfResPath = ctcGetBaseUrl()
								+ "/download/resources/ru.mail.plugins.clipcopier.link-copy-to-clipboard:ctc-web-resources/zeroclipboard.swf";
						...
						zcClient.on("ready", function(readyEvent) {
							zcClient.on("copy", function(event) {
								...
								if (clipHolder.hasClass('ctc-column-view')) {
									clipboardData = clipHolder
											.attr("ctc-value");
								} else {
									clipboardData = jQuery("#key-val").prop(
											'textContent')
											+ " "
											+ jQuery("#summary-val").prop(
													'textContent');
								}
					...
					function attachCtcToOpsBar() {
						...
						if (toolbarItems.find(".type-link-copytoclip").length <= 0) {
							...
							var ctcOpsItemHtml = "<li id='"
									+ opsBarCtcItemId
									+ "' class='toolbar-item'><span title='Copy to clipboard' class='toolbar-trigger type-link-copytoclip'>&copy;</span></li>";
